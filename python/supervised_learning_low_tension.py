...
for name, regressor in regressors.items():
    ...
    random_search.fit(train_df.drop(columns=['energia_ativa_baixa_tensao_kwh']), train_df['energia_ativa_baixa_tensao_kwh'])
    ...
    y_pred = best_model.predict(test_df.drop(columns=['energia_ativa_baixa_tensao_kwh']))
    mse = mean_squared_error(test_df['energia_ativa_baixa_tensao_kwh'], y_pred)
    mae = mean_absolute_error(test_df['energia_ativa_baixa_tensao_kwh'], y_pred)
    r2 = r2_score(test_df['energia_ativa_baixa_tensao_kwh'], y_pred)
...
with open('best_hyperparameters_and_scores_randomized_low_tensions.txt', 'w') as f:
